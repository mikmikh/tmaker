  ...
  _init() {
    ...
    this.rootEl.appendChild(
      JElementBuilder.addButton("rename", () => {
        this.store.dispatch({ type: ACTION_TYPES.SPRITE_RENAME });
      }),
    );
    this.rootEl.appendChild(
      JElementBuilder.addButton("add", () => {
        this.store.dispatch({ type: ACTION_TYPES.SPRITE_ADD });
      }),
    );
    this.rootEl.appendChild(
    ...
    this.rootEl.appendChild(
      JElementBuilder.addButton("delete", () => {
        this.store.dispatch({ type: ACTION_TYPES.SPRITE_DELETE });
      }),
    );

    {
      this.frameCountEl = JElementBuilder.addInput(
        ...
        (e) => {
          ...
          img.src = url;
          img.onload = () => {
            this.jcanvas.drawImage(img, [0, 0], prevSize, [0, 0], prevSize);
          };
        },
      );
